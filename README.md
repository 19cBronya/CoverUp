...
- 批量扫描目录中的视频文件（支持递归）
- 手动指定封面图并直接写入视频
- 支持三种封面处理策略（GUI 下拉框直接切换）
  - `元数据封面（失败跳过）`：写入元数据封面，失败则跳过
  - `元数据封面（失败后首帧）`：先尝试元数据，失败后自动回退到首帧重编码
  - `直接首帧`：跳过元数据，直接 GPU 加速重编码首帧
- 按分钟窗口抽帧，自动生成候选封面
...
如果追求兼容性（如 AVI/FLV 等不支持元数据的格式），或想确保封面一定可见，请使用 `直接首帧` 策略。
...
直接替换封面（首帧重编码，自动 GPU 加速）：

```bash
coverup-cli --video ./videos/a.mp4 --cover ./covers/a.jpg --mode first_frame
```
